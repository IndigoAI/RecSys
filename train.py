...
args.hidden_size = 64
args.path = "gdrive/MyDrive/HSE/recsys/best_model/"
# args.model_type = 'MIND'
...
def get_model(dataset, model_type, item_count, batch_size, maxlen):
    ...
    elif model_type == 'ComiRec-SA-with-loss':
        model = Model_ComiRec_SA(item_count, args.embedding_dim, args.hidden_size, batch_size, args.num_interest, maxlen, with_loss=True)
    else:
...
def train(
    ...
    best_model_path = "gdrive/MyDrive/HSE/recsys/best_model/" + exp_name + '/'
    ...
    wandb.login()
    wandb.init(project=model_type, name=exp_name, config=params)
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        model.restore(sess, 'gdrive/MyDrive/HSE/recsys/best_model/taobao_ComiRec-SA_b256_lr0.001_d64_len50_0/')
        ...
        try:
            loss_sum = 0.0
            trials = 0
            for src, tgt in tqdm(train_data, total=max_iter * 1000):
                ...
                if iter % test_iter == 0:
                    ...
                    wandb.log({'train/loss': loss_sum / test_iter, 'iters': iter})
                    if metrics != {}:
                        for key, value in metrics.items():
                            wandb.log({f'eval/{key}': value, 'iters': iter})
...
def output(test_file,
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
        model.restore(sess, best_model_path)
        test_data = DataIterator(test_file, 1, maxlen, train_flag=2)

        item_embs = model.output_item(sess)  
        src, tgt = next(test_data)
        nick_id, item_id, hist_item, hist_mask = prepare_data(src, tgt)
        test_embs = item_embs[hist_item]
        
        seq_attn_weight = model.seq_attn_weight
        return seq_attn_weight
